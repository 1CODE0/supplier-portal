...
import { Button, Paper, Typography, Box, CircularProgress, Autocomplete, TextField, MenuItem } from '@mui/material'
...
export function getEnumKeyByValue<T extends { [key: string]: string | number }>(enumObj: T, value: T[keyof T]) {
...
export default function NewOrderPage() {
  ...
  const onSubmit = (data: OrderInput) => {
    const payload = {
      ...
      status: data.status
        ? (getEnumKeyByValue(Order.status, data.status as Order.status) as Order.status)
    ...
    <Paper sx={{ p: 4, maxWidth: 600, mx: 'auto', mt: 4 }}>
      ...
      <Box component='form' onSubmit={handleSubmit(onSubmit)} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {id ? (
          <Controller
            ...
            render={({ field, fieldState: { error } }) => (
              <TextField {...field} select label='Status' fullWidth error={!!error} helperText={error?.message}>
        ...
        <Box sx={{ position: 'relative', width: 'fit-content', mt: 2 }}>
          ...
          >
            {busy ? (id ? 'Updating' : 'Creating') + ' Order…' : (id ? 'Update' : 'Create') + ' Order'}
          </Button>
        </Box>
      </Box>
    </Paper>
